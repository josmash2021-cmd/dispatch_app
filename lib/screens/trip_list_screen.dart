...
class _TripListScreenState extends State<TripListScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Row(children: [
          Container(width: 32, height: 32, decoration: BoxDecoration(color: AppColors.primary, borderRadius: BorderRadius.circular(8)), child: const Icon(Icons.local_taxi_rounded, color: Color(0xFF08090C), size: 18)),
          const SizedBox(width: 10),
          const Text('Trips', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w700, color: AppColors.textPrimary)),
          const SizedBox(width: 10),
              ...
              child: TextField(
                ...
                decoration: InputDecoration(
                  ...
                  suffixIcon: _searchController.text.isNotEmpty
                      ? IconButton(icon: const Icon(Icons.clear_rounded, color: AppColors.textHint), onPressed: () { _searchController.clear(); trips.setSearchQuery(''); })
                      : null,
                  filled: true,
  ...
  Widget _buildBody(TripProvider trips) {
    ...
    if (trips.errorMessage != null) {
      return Center(
        child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          ...
          Text(trips.errorMessage!, textAlign: TextAlign.center, style: const TextStyle(color: AppColors.textSecondary)),
          const SizedBox(height: 16),
          ElevatedButton(onPressed: trips.refreshTrips, child: const Text('Retry')),
        ]),
      );
    }
    if (trips.trips.isEmpty) {
      return Center(
        child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          Icon(trips.statusFilter != null ? Icons.filter_list_off_rounded : Icons.local_taxi_outlined, size: 56, color: AppColors.textHint),
          const SizedBox(height: 12),
          Text(
            trips.statusFilter != null ? 'No trips match this filter' : 'No trips yet',
        ...
        child: Row(mainAxisSize: MainAxisSize.min, children: [
          Container(
            ...
            decoration: BoxDecoration(
              ...
              boxShadow: [BoxShadow(color: AppColors.error.withValues(alpha: _anim.value * 0.6), blurRadius: 4)],
            ),
          ),
          ...
          Text(
            ...
            style: TextStyle(
              ...
              color: AppColors.error.withValues(alpha: 0.7 + 0.3 * _anim.value),
              letterSpacing: 0.5,
            ),
          ),
        ]),
